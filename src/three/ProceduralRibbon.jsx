...
export function ProceduralRibbon({ scrollRef, pointerRef, intensity = 1 }) {
  const mesh = useRef(null)
  const baseColor = useMemo(() => new Color('#f6f5f0'), [])
  const accentColor = useMemo(() => new Color('#4b91ff'), [])
  const baseEmissive = useMemo(() => new Color('#050505'), [])

  const geometry = useMemo(() => {
    const points = [
      new Vector3(-1.6, 0.2, -0.6),
      new Vector3(-0.6, 1.1, 0.4),
      new Vector3(0.6, 0.6, -0.2),
      new Vector3(1.4, -0.4, 0.6),
      new Vector3(0.2, -1.2, -0.5),
      new Vector3(-1.2, -0.6, 0.2)
    ]
    const curve = new CatmullRomCurve3(points, true, 'catmullrom', 0.6)
    return new TubeGeometry(curve, 220, 0.12, 24, true)
  }, [])
  ...
  useFrame((state, delta) => {
    ...
    const pose = getPose(modelStages.ribbon, index, t)
    const pointer = pointerRef.current
    const float = (progress - 0.5) * 0.12 * intensity
    const targetRotationX = pose.rotation[0] + pointer.y * 0.1 * intensity
    const targetRotationY = pose.rotation[1] + pointer.x * 0.14 * intensity
    ...
    material.roughness = MathUtils.lerp(0.28, 0.16, energy)
    material.metalness = MathUtils.lerp(0.85, 0.95, energy)
